...
pipeline = import_file('dump-liquid.lammpstrj')
pipeline.source.data.cell_.is2D = True
modifier = CoordinationAnalysisModifier(cutoff = 12.0, number_of_bins = 200)
pipeline.modifiers.append(modifier)
rdfdata = pipeline.compute(100)
export_file(rdfdata,'rdf-pure_liquid.txt','txt/table')
!
...
export_file(qdata,'last-snap-pure_liquid.data','lammps/data')
!
...
mv log.lammps log-q-liquid.lammps
